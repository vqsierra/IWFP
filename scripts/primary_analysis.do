...
// PRIMARY ANALYSIS - LASSO Logistic Regression  
// Predicting Suicidal Ideation Among Native American High School Students (CHKS Dataset)
// Author: Valentín Quiroz Sierra, PhD, MSW  
// Last Updated: March 2025  
// Description: Prepares dataset and runs a LASSO logistic regression with 10-fold cross-validation  
// to predict past-year suicidal ideation using CHKS data.
...
* Define paired lists: original and reverse-coded variable names
...
* Loop to generate reverse-coded versions
forvalues i = 1/8 {
...
* Compute composite and standardized score
...
// ---------------------------
predict double si_estimatesPR3, pr
calibrationbelt si si_estimatesPR3, devel("internal") cLevel1(0.95) cLevel2(0.99) maxDeg(4) thres(0.95)
cvauroc si si_estimatesPR3, kfold(10) seed(1533) fit detail graphlowess
brier si si_estimatesPR3
sum si_estimatesPR3, detail
